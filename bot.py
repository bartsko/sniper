...
# Ładujemy listing z JSON
def load_listing(path="listings.json"):
    with open(path) as f:
        listing = json.load(f)
    # Wyciągamy pierwsze zadanie z listy
...
async def main():
    ...
    listing_time = listing["listing_time"]   # ISO8601+offset, np. "2025-07-11T21:15:00+02:00"

    # 1. Synchronizacja czasu
    offset = await get_server_offset()
    print(f"[SYNC] offset serwera: {offset} ms")

    # 2. Czekaj do 3s przed listingiem
    ...
    # 6. Wyślij 3 MARKET BUY: T0-10ms, T0-5ms, T0
