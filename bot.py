...
async def main():
    ...
    async with httpx.AsyncClient(http2=True) as client:
        ...
        dt = datetime.fromisoformat(LISTING_TIME).astimezone(timezone.utc)
        ...
        if build["mode"] == "limit":
            ...
            async with websockets.connect(WS_URL) as ws:
                await ws.send(json.dumps({"method":"SUBSCRIBE","params":[f"{SYMBOL.lower()}@trade"],"id":1}))
                while True:
                    msg = await ws.recv()
                    data = json.loads(msg)
                    if data.get("e") == "trade":
