...
async def prepare_buy(client):
    ...
    if not asks:
        ...
        return None  # Brak cen limit - będzie fallback do market
...
async def multi_attempts(client, build, offset, t0_local, amount):
    attempts = []
    bought = 0
    rem = amount
    for i, sa in enumerate((t0_local-10, t0_local-5, t0_local)):
        if rem <= 0:
            break
        if build["mode"] == "limit":
        ...
        if att["status"] == "OK" and att["exec_qty"] > 0:
            bought += att["exec_qty"] * (build.get("limit_price") or 1)
            break  # Po pierwszym sukcesie przerywamy kolejne próby
    return attempts, bought
...
async def main():
    ...
    async with httpx.AsyncClient(http2=True) as client:
        ...
        while int(time.time()*1000) < t0 - 10000: await asyncio.sleep(0.001)
        await warmup(client)
        build = await prepare_buy(client)
        t0_local = t0
        attempts, bought = [], 0

        # Jeśli nie ma orderbooka — próbuj market!
        if not build:
            build = {"mode": "market"}
            attempts, bought = await multi_attempts(client, build, offset, t0_local, QUOTE_AMOUNT)
        else:
            attempts, bought = await multi_attempts(client, build, offset, t0_local, QUOTE_AMOUNT)
            if bought <= 0:
                print("[BOT] Próba market (orderbook pusty/lub fail limitów)...")
                build = {"mode": "market"}
                market_attempts, market_bought = await multi_attempts(client, build, offset, t0_local, QUOTE_AMOUNT)
                attempts += market_attempts
                bought += market_bought
        ...
        # SELL
        sell_price = round((build.get("limit_price") or 0) * (1 + PROFIT_PCT / 100), 8) if build.get("limit_price") else 0
        sell_qty = bought / (build.get("limit_price") or 1) if build.get("limit_price") else bought
